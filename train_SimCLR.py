...
parser.add_argument('--weight-decay', default=1e-6, type=float, metavar='W',
                    help='weight decay')
parser.add_argument('--lambd', default=0.001, type=float, metavar='L',
                    help='weight on off-diagonal terms')
parser.add_argument('--projector', default='512-256-128', type=str,
                    metavar='MLP', help='projector MLP')
parser.add_argument('--print-freq', default=20, type=int, metavar='N',
                    help='print frequency')
parser.add_argument('--checkpoint-dir', default='./cpt_simclr_cifar10/', type=Path,
...
def main_worker(gpu, args):
    ...
    if args.rank == 0:
        ...
        torch.save(model.module.backbone.state_dict(),
                   args.checkpoint_dir / 'simclr_cifar100.pth')
...
class SimCLR(nn.Module):
    ...
    def forward(self, y1, y2):
        z1, z2 = self.projector(self.backbone(y1)), self.projector(self.backbone(y2))
        z1, z2 = self.bn(z1), self.bn(z2)
        z1_m = torch.sqrt(torch.sum(z1.clone().pow_(2),dim=1)).expand(128,128).T
        z2_m = torch.sqrt(torch.sum(z2.clone().pow_(2),dim=1)).expand(128,128).T
